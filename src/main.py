...
def extract_modpack(tmp_path, file_name):
	with zipfile.ZipFile(file_name, 'r') as zip_ref:
		zip_ref.extractall(tmp_path)

def launch_forge_installer(tmp_path):
	p = subprocess.Popen((f'java -jar {tmp_path}\\Modpack\\forge-1.12.2-14.23.5.2860-installer.jar'))
	p.wait()

def update_launcher_profiles(game_path):
	profile_json_path = f"{game_path}\\launcher_profiles.json"
	with codecs.open(profile_json_path, 'r+', encoding='utf-8') as stream:
		...
		data['profiles']['MPAP'] = {
			...
			"gameDir" : f"{game_path}\\profiles\\MPAP",
...
def copy_mods(modpack_path, game_path):
	dist_mods_folder = f"{game_path}\\profiles\\MPAP\\mods"
	os.makedirs(dist_mods_folder, exist_ok=True)
	shutil.rmtree(dist_mods_folder)
	shutil.copytree(f"{modpack_path}\\mods", dist_mods_folder)
